<?php
	...
	$id = $_GET["id"];
	if ($id > 0){
		$data_memo = sqlsrv_fetch_array(sqlsrv_query($sqlconn_db2, "select mfrom,msubject,mbody from j2365join_memo where id = '".$id ."'"), SQLSRV_FETCH_ASSOC);
		$data_memo["mbody"]=str_Replace("<script","<scrpt",$data_memo["mbody"]);
		$data_memo["msubject"]=str_Replace("<script","<scrpt",$data_memo["msubject"]);
		sqlsrv_query($sqlconn_db2, "update j2365join_memo set mread='1' where id = '".$id."'");
	}
?>
