...
$sqlu = sqlsrv_fetch_array(sqlsrv_query($sqlconn, "select bankname, bankaccno, bankaccname, bankgrup,playerpt,xdeposit from u6048user_id where loginid ='".$login."'"), SQLSRV_FETCH_ASSOC);
                                ...
                                <form class="form-horizontal" role="form" method="POST">
									...
									<?php
										if ($playerpt == 1){
											...
											$sql		= sqlsrv_fetch_array(sqlsrv_query($sqlconn, "select userprefix from u6048user_data where userid = '".$login."' "), SQLSRV_FETCH_ASSOC);
											$userx		= $sql["userprefix"];
										...
										for($i=0; $i<$jumbank; $i++){
											...
											/*if ($status == 1){
												$condition = "<span id=text-bank style=color:green>PROSES DEPOSIT CEPAT ( 5 - 10 menit )</span>";
											}else if ($status == 2){
												$condition = "<span id=text-bank style=color:yellow>PROSES DEPOSIT LAMBAT ( 20 - 45 menit )</span>";
											}else if ($status == 3){
												$condition = "<span id=text-bank style=color:red>PROSES DEPOSIT SANGAT LAMBAT ( 2 - 3 jam )</span>";
											}
											
											if ($i == ($jumbank - 1)){
												$line_color = "";
											}else{
												$line_color = "border-bottom:1px solid #3a0d0d";
											}*/
											...
											echo "	<tr>
													...
													<td>
														&nbsp;<span><img src='assets/img/$link_img/".strtolower($tempRow["bank"]).".png'></span>
													</td>
													<td>
