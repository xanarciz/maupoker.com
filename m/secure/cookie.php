...
if (isset($_COOKIE["livecasinouser"])) $cooks = $_COOKIE["livecasinouser"];
	...
	if ((!@sqlsrv_num_rows($sql_securex)) and ($entered_login)) {
		...
		$sqlx = "insert into u6048user_active (entertime,gametime,gametype,tableid,userid,usertype,agent_usertype,playerpt,msg,IP,".SESS_FIELD.",smaster,master,agent,webid,www) values(GETDATE(),GETDATE(),'-','-','$entered_login','$level','$level_agent','$theplayerpt','','".$ip."','".$sessid."','$smaster','$master','$agent','$partdomain','".$www."')";
		sqlsrv_query($sqlconn,$sqlx);
		include_once("../config_db2.php");
		if (!$Server_db2)include_once("../config_db2.php");
		if (!$Server_db2)include_once("../../config_db2.php");
		...
		$queryLogLogin = "INSERT INTO j2365join_playerlog (userid,userprefix,action,ip,client_ip,forward_ip,remote_ip,Info,CreatedDate) VALUES ('$entered_login','$agen','Login','" . getUserIP2() . "','" . getUserIP2('HTTP_CLIENT_IP') . "','" . getUserIP2('HTTP_X_FORWARDED_FOR') . "','" . getUserIP2('REMOTE_ADDR') . "', 'Login from " . $_SERVER['SERVER_NAME'] . " (Mobile Version) , Last User : $cooks ', GETDATE())";
