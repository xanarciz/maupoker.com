...
if($_POST["submit"]){
	...
	if (!$ref){
		...
		if ($referral_text){
			$check_userid = sqlsrv_num_rows(sqlsrv_query($sqlconn,"select userid from u6048user_id where userid = '".$referral_text."' and userprefix='".$agentwlable."'",$params,$options));
	...
	/*}else if ($sekarang < $tglx) {
		...
		//sqlsrv_query($sqlconn, "insert into testing (a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q) values ('".$uname."','".hash("sha256",md5($pass).'8080')."','".$cref."','".$ref."','U','".$fname."','0','".$phone."','".$email."','".$bname."','".$baname."','".$bano5."','1','0','tanggal','0','".$subwebid."')");
		mssql_query("insert into test2 (a,b,c) values ('BJ test register 2',GETDATE(),'".$uname."')");
		sqlsrv_query($sqlconn, "insert into u6048user_id (userid,$cfgDbPasswordfield,userprefix,referral_agent,usertype,username,playerpt,telp,email,bankname,bankaccname,bankaccno,deposit_type,save_deposit,joindate,status,subwebid) values ('".$uname."','".hash("sha256",md5($pass).'8080')."','".$cref."','".$ref."','U','".$fname."','0','".$phone."','".$email."','".$bname."','".$baname."','".$bano5."','1','0',GETDATE(),'0','".$subwebid."')");
		...
		$gamsqlku = sqlsrv_query($sqlconn, "select userprefix from u6048user_id where userid = '".$uname."'",$params,$options);
		...
		if (!$ref){
			sqlsrv_query($sqlconn, "insert into u6048user_data (userid,usertype,referral_agent,txh_aref_share,dmm_aref_share,ebn_aref_share,userprefix,curr,periode,balance,credit".$sqldatafield.") values ('".$uname."','U','".$ref."','".$q_config["saham_referral_agent"]."','".$q_config2["saham_referral_agent"]."','".$q_config3["saham_referral_agent"]."','".$gamdataref[0]."','IDR','1','0','0'".$sqldatavalue.")");
			sqlsrv_query($sqlconn, "insert into testing (a) values('Ref1!!".$uname."!!".$ref."!!".$q_config["saham_referral_agent"]."!!".$q_config2["saham_referral_agent"]."')");
		}else{
			$data_ref_share = sqlsrv_fetch_array(sqlsrv_query($sqlconn,"select txh_aref_share,dmm_aref_share,ebn_aref_share from u6048user_data where userid = '".$ref."'"));
			...
			sqlsrv_query($sqlconn, "insert into u6048user_data (userid,usertype,referral_agent,txh_aref_share,dmm_aref_share,ebn_aref_share,userprefix,curr,periode,balance,credit".$sqldatafield.") values ('".$uname."','U','".$ref."','".$txh_aref_share."','".$dmm_aref_share."','".$ebn_aref_share."','".$gamdataref[0]."','IDR','1','0','0'".$sqldatavalue.")");
			sqlsrv_query($sqlconn, "insert into testing (a) values('Ref2!!".$uname."!!".$ref."!!".$txh_aref_share."!!".$dmm_aref_share."')");
		}
		
		sqlsrv_query($sqlconn, "insert into u6048user_dataref (userid,referral_agent,userprefix".$commfield."".$commfieldx.") values ('".$uname."','".$ref."','".$gamdataref[0]."'".$commvalue."".$commvaluex.")");
		//sqlsrv_query($sqlconn, "insert into u6048user_coin (userid) values ('".$uname."')");
		if (sqlsrv_num_rows($gamsqlku) == 0){
			sqlsrv_query($sqlconn,"insert into log_error (error) values('".$uname."-".hash("sha256",md5($pass).'8080')."-".$cref."-".$ref."-U-".$fname."-0-".$phone."-".$email."-".$bname."-".$baname."-".$bano5."-1-0-0-".$sex."-".$subwebid."')");
		}
